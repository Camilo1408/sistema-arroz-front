...
import { Upload, ImageIcon, Film, CheckCircle2, Cpu } from "lucide-react";
...
export function FileUpload({ onFileSelected, accept = "both", isLoading = false }) {
  ...
  return (
    ...
    >
      {/* Shimmer overlay while dragging */}
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center gap-2 py-1">
          ...
          <div>
            ...
            <p className="text-xs text-stone-400 mt-0.5">Clic para cambiar archivo</p>
          </div>
        </div>
      ) : (
        <div className="flex flex-col items-center gap-3 py-1">
          <div className="flex items-center gap-2 text-stone-400">
            {(accept === "image" || accept === "both") && <ImageIcon className="w-6 h-6" />}
            {(accept === "video" || accept === "both") && <Film className="w-6 h-6" />}
            <Upload className="w-5 h-5" />
          </div>
          <div>
            <p className="text-sm text-stone-600">
              Arrastra aquí o{" "}
              <span className="text-green-600 font-semibold underline underline-offset-2">selecciona un archivo</span>
            </p>
            <p className="text-xs text-stone-400 mt-1">
              {accept === "image" ? "JPG, PNG" : accept === "video" ? "MP4, WebM" : "JPG, PNG, MP4"} · Máx. 50 MB
            </p>
          </div>
        </div>
      )}
